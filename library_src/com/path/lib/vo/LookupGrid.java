package com.path.lib.vo;

public class LookupGrid
{

    protected String securityRef;
    protected String accesskey;
    protected String altClass;
    protected String altRows;
    protected String autoencode;
    protected String caption;
    protected String cellEdit;
    protected String cellurl;
    protected String connectWith;
    protected String cssClass;
    protected String cssErrorClass;
    protected String cssErrorStyle;
    protected String cssStyle;
    protected String dataType = "json";
    protected String direction;
    protected String disabled;
    protected String draggable;
    protected String draggableAddClasses;
    protected String draggableAppendTo;
    protected String draggableAxis;
    protected String draggableCancel;
    protected String draggableContainment;
    protected String draggableCursor;
    protected String draggableDelay;
    protected String draggableDistance;
    protected String draggableHandle;
    protected String draggableHelper;
    protected String draggableIframeFix;
    protected String draggableOnDragTopics;
    protected String draggableOnStartTopics;
    protected String draggableOnStopTopics;
    protected String draggableOpacity;
    protected String draggableRefreshPositions;
    protected String draggableRevert;
    protected String draggableRevertDuration;
    protected String draggableScope;
    protected String draggableScroll;
    protected String draggableScrollSensitivity;
    protected String draggableScrollSpeed;
    protected String draggableSnap;
    protected String draggableSnapMode;
    protected String draggableSnapTolerance;
    protected String draggableZindex;
    protected String droppable;
    protected String droppableAccept;
    protected String droppableActiveClass;
    protected String droppableAddClasses;
    protected String droppableGreedy;
    protected String droppableHoverClass;
    protected String droppableOnActivateTopics;
    protected String droppableOnDeactivateTopics;
    protected String droppableOnDropTopics;
    protected String droppableOnOutTopics;
    protected String droppableOnOverTopics;
    protected String droppableScope;
    protected String droppableTolerance;
    protected String editinline = "true";
    protected String editurl;
    protected String effect;
    protected String effectDuration;
    protected String effectMode;
    protected String effectOptions;
    protected String errorElementId;
    protected String errorText;
    protected boolean filter = true;
    protected String filterOptions;
    protected String footerrow;
    protected String formIds;
    protected String gridModel = "gridModel";
    protected String groupCollapse;
    protected String groupColumnShow;
    protected String groupDataSorted;
    protected String groupField;
    protected String groupMinusIcon;
    protected String groupOrder;
    protected String groupPlusIcon;
    protected String groupShowSummaryOnHide;
    protected String groupSummary;
    protected String groupText;
    protected String height;
    protected String hiddengrid;
    protected String hidegrid;
    protected String hoverrows;
    protected String href;
    protected String id;
    protected String indicator;
    protected String javascriptTooltip;
    protected String key;
    protected String label;
    protected String labelSeparator;
    protected String labelposition;
    protected String listenTopics;
    protected String loadingText;
    protected String loadonce;
    protected String multiboxonly;
    protected String multiselect;
    protected String multiselectWidth;
    protected String name;
    protected String navigator = "true";
    protected String navigatorAdd;
    protected String navigatorAddOptions;
    protected String navigatorDelete;
    protected String navigatorDeleteOptions;
    protected String navigatorEdit;
    protected String navigatorEditOptions;
    protected String navigatorRefresh;
    protected String navigatorSearch;
    protected String navigatorSearchOptions = "{closeOnEscape: true,closeAfterSearch: true,multipleSearch: true}";
    protected String navigatorView;
    protected String navigatorViewOptions;
    protected String onAlwaysTopics;
    protected String onBeforeTopics;
    protected String onBlurTopics;
    protected String onCellEditErrorTopics;
    protected String onCellEditSuccessTopics;
    protected String onCellSelectTopics;
    protected String onChangeTopics;
    protected String onCompleteTopics;
    protected String onDisableTopics;
    protected String onEditInlineAfterSaveTopics;
    protected String onEditInlineBeforeTopics;
    protected String onEditInlineErrorTopics;
    protected String onEditInlineSuccessTopics;
    protected String onEffectCompleteTopics;
    protected String onEnableTopics;
    protected String onErrorTopics = "loadError";
    protected String onFocusTopics;
    protected String onGridCompleteTopics;
    protected String onPagingTopics;
    protected String onSelectAllTopics;
    protected String onSelectRowTopics;
    protected String onSortColTopics;
    protected String onSuccessTopics;
    protected String onblur;
    protected String onchange;
    protected String onclick;
    protected String ondblclick;
    protected String onfocus;
    protected String onkeydown;
    protected String onkeypress;
    protected String onkeyup;
    protected String onmousedown;
    protected String onmousemove;
    protected String onmouseout;
    protected String onmouseover;
    protected String onmouseup;
    protected String onselect;
    protected String openTemplate;
    protected String page;
    protected String pager = "true";
    protected String pagerButtons;
    protected String pagerInput;
    protected String pagerPosition;
    protected String prmNames;
    protected String recordpos;
    protected String reloadTopics;
    protected String requestType;
    protected String required;
    protected String requiredposition;
    protected String resizable;
    protected String resizableAnimate;
    protected String resizableAnimateDuration;
    protected String resizableAnimateEasing;
    protected String resizableAspectRatio;
    protected String resizableAutoHide;
    protected String resizableContainment;
    protected String resizableDelay;
    protected String resizableDistance;
    protected String resizableGhost;
    protected String resizableHandles;
    protected String resizableHelper;
    protected String resizableMaxHeight;
    protected String resizableMaxWidth;
    protected String resizableMinHeight;
    protected String resizableMinWidth;
    protected String resizableOnResizeTopics;
    protected String resizableOnStartTopics;
    protected String resizableOnStopTopics;
    protected String[] rowList = new String[] { "5", "10", "15", "20" };
    protected String rowNum = "10";
    protected String rowTotal;
    protected String rownumbers;
    protected String scroll;
    protected String scrollrows;
    protected String shrinkToFit = "false";
    protected String sortable;
    protected String sortableAppendTo;
    protected String sortableAxis;
    protected String sortableCancel;
    protected String sortableConnectWith;
    protected String sortableContainment;
    protected String sortableCursor;
    protected String sortableCursorAt;
    protected String sortableDelay;
    protected String sortableDistance;
    protected String sortableDropOnEmpty;
    protected String sortableForceHelperSize;
    protected String sortableForcePlaceholderSize;
    protected String sortableGrid;
    protected String sortableHandle;
    protected String sortableHelper;
    protected String sortableItems;
    protected String sortableOnActivateTopics;
    protected String sortableOnBeforeStopTopics;
    protected String sortableOnChangeTopics;
    protected String sortableOnDeactivateTopics;
    protected String sortableOnOutTopics;
    protected String sortableOnOverTopics;
    protected String sortableOnReceiveTopics;
    protected String sortableOnRemoveTopics;
    protected String sortableOnSortTopics;
    protected String sortableOnStartTopics;
    protected String sortableOnStopTopics;
    protected String sortableOnUpdateTopics;
    protected String sortableOpacity;
    protected String sortablePlaceholder;
    protected String sortableRevert;
    protected String sortableScroll;
    protected String sortableScrollSensitivity;
    protected String sortableScrollSpeed;
    protected String sortableTolerance;
    protected String sortableZindex;
    protected String sortname;
    protected String sortorder;
    protected String subGridUrl;
    protected String subGridWidth;
    protected String tabindex;
    protected String targets;
    protected String template;
    protected String templateDir;
    protected String timeout;
    protected String title;
    protected String tooltip;
    protected String tooltipConfig;
    protected String tooltipCssClass;
    protected String tooltipDelay;
    protected String tooltipIconPath;
    protected String userDataOnFooter;
    protected String value;
    protected Boolean viewrecords = true;
    protected String width;
    protected String url = "plookupGrid";
    protected String searchColumns ;// list of search columns comma separated
    protected String columnsOrder;
    public String getSecurityRef()
    {
	return securityRef;
    }

    public void setSecurityRef(String securityRef)
    {
	this.securityRef = securityRef;
    }

    public String getAccesskey()
    {
	return accesskey;
    }

    public void setAccesskey(String accesskey)
    {
	this.accesskey = accesskey;
    }

    public String getAltClass()
    {
	return altClass;
    }

    public void setAltClass(String altClass)
    {
	this.altClass = altClass;
    }

    public String getAltRows()
    {
	return altRows;
    }

    public void setAltRows(String altRows)
    {
	this.altRows = altRows;
    }

    public String getAutoencode()
    {
	return autoencode;
    }

    public void setAutoencode(String autoencode)
    {
	this.autoencode = autoencode;
    }

    public String getCaption()
    {
	return caption;
    }

    public void setCaption(String caption)
    {
	this.caption = caption;
    }

    public String getCellEdit()
    {
	return cellEdit;
    }

    public void setCellEdit(String cellEdit)
    {
	this.cellEdit = cellEdit;
    }

    public String getCellurl()
    {
	return cellurl;
    }

    public void setCellurl(String cellurl)
    {
	this.cellurl = cellurl;
    }

    public String getConnectWith()
    {
	return connectWith;
    }

    public void setConnectWith(String connectWith)
    {
	this.connectWith = connectWith;
    }

    public String getCssClass()
    {
	return cssClass;
    }

    public void setCssClass(String cssClass)
    {
	this.cssClass = cssClass;
    }

    public String getCssErrorClass()
    {
	return cssErrorClass;
    }

    public void setCssErrorClass(String cssErrorClass)
    {
	this.cssErrorClass = cssErrorClass;
    }

    public String getCssErrorStyle()
    {
	return cssErrorStyle;
    }

    public void setCssErrorStyle(String cssErrorStyle)
    {
	this.cssErrorStyle = cssErrorStyle;
    }

    public String getCssStyle()
    {
	return cssStyle;
    }

    public void setCssStyle(String cssStyle)
    {
	this.cssStyle = cssStyle;
    }

    public String getDataType()
    {
	return dataType;
    }

    public void setDataType(String dataType)
    {
	this.dataType = dataType;
    }

    public String getDirection()
    {
	return direction;
    }

    public void setDirection(String direction)
    {
	this.direction = direction;
    }

    public String getDisabled()
    {
	return disabled;
    }

    public void setDisabled(String disabled)
    {
	this.disabled = disabled;
    }

    public String getDraggable()
    {
	return draggable;
    }

    public void setDraggable(String draggable)
    {
	this.draggable = draggable;
    }

    public String getDraggableAddClasses()
    {
	return draggableAddClasses;
    }

    public void setDraggableAddClasses(String draggableAddClasses)
    {
	this.draggableAddClasses = draggableAddClasses;
    }

    public String getDraggableAppendTo()
    {
	return draggableAppendTo;
    }

    public void setDraggableAppendTo(String draggableAppendTo)
    {
	this.draggableAppendTo = draggableAppendTo;
    }

    public String getDraggableAxis()
    {
	return draggableAxis;
    }

    public void setDraggableAxis(String draggableAxis)
    {
	this.draggableAxis = draggableAxis;
    }

    public String getDraggableCancel()
    {
	return draggableCancel;
    }

    public void setDraggableCancel(String draggableCancel)
    {
	this.draggableCancel = draggableCancel;
    }

    public String getDraggableContainment()
    {
	return draggableContainment;
    }

    public void setDraggableContainment(String draggableContainment)
    {
	this.draggableContainment = draggableContainment;
    }

    public String getDraggableCursor()
    {
	return draggableCursor;
    }

    public void setDraggableCursor(String draggableCursor)
    {
	this.draggableCursor = draggableCursor;
    }

    public String getDraggableDelay()
    {
	return draggableDelay;
    }

    public void setDraggableDelay(String draggableDelay)
    {
	this.draggableDelay = draggableDelay;
    }

    public String getDraggableDistance()
    {
	return draggableDistance;
    }

    public void setDraggableDistance(String draggableDistance)
    {
	this.draggableDistance = draggableDistance;
    }

    public String getDraggableHandle()
    {
	return draggableHandle;
    }

    public void setDraggableHandle(String draggableHandle)
    {
	this.draggableHandle = draggableHandle;
    }

    public String getDraggableHelper()
    {
	return draggableHelper;
    }

    public void setDraggableHelper(String draggableHelper)
    {
	this.draggableHelper = draggableHelper;
    }

    public String getDraggableIframeFix()
    {
	return draggableIframeFix;
    }

    public void setDraggableIframeFix(String draggableIframeFix)
    {
	this.draggableIframeFix = draggableIframeFix;
    }

    public String getDraggableOnDragTopics()
    {
	return draggableOnDragTopics;
    }

    public void setDraggableOnDragTopics(String draggableOnDragTopics)
    {
	this.draggableOnDragTopics = draggableOnDragTopics;
    }

    public String getDraggableOnStartTopics()
    {
	return draggableOnStartTopics;
    }

    public void setDraggableOnStartTopics(String draggableOnStartTopics)
    {
	this.draggableOnStartTopics = draggableOnStartTopics;
    }

    public String getDraggableOnStopTopics()
    {
	return draggableOnStopTopics;
    }

    public void setDraggableOnStopTopics(String draggableOnStopTopics)
    {
	this.draggableOnStopTopics = draggableOnStopTopics;
    }

    public String getDraggableOpacity()
    {
	return draggableOpacity;
    }

    public void setDraggableOpacity(String draggableOpacity)
    {
	this.draggableOpacity = draggableOpacity;
    }

    public String getDraggableRefreshPositions()
    {
	return draggableRefreshPositions;
    }

    public void setDraggableRefreshPositions(String draggableRefreshPositions)
    {
	this.draggableRefreshPositions = draggableRefreshPositions;
    }

    public String getDraggableRevert()
    {
	return draggableRevert;
    }

    public void setDraggableRevert(String draggableRevert)
    {
	this.draggableRevert = draggableRevert;
    }

    public String getDraggableRevertDuration()
    {
	return draggableRevertDuration;
    }

    public void setDraggableRevertDuration(String draggableRevertDuration)
    {
	this.draggableRevertDuration = draggableRevertDuration;
    }

    public String getDraggableScope()
    {
	return draggableScope;
    }

    public void setDraggableScope(String draggableScope)
    {
	this.draggableScope = draggableScope;
    }

    public String getDraggableScroll()
    {
	return draggableScroll;
    }

    public void setDraggableScroll(String draggableScroll)
    {
	this.draggableScroll = draggableScroll;
    }

    public String getDraggableScrollSensitivity()
    {
	return draggableScrollSensitivity;
    }

    public void setDraggableScrollSensitivity(String draggableScrollSensitivity)
    {
	this.draggableScrollSensitivity = draggableScrollSensitivity;
    }

    public String getDraggableScrollSpeed()
    {
	return draggableScrollSpeed;
    }

    public void setDraggableScrollSpeed(String draggableScrollSpeed)
    {
	this.draggableScrollSpeed = draggableScrollSpeed;
    }

    public String getDraggableSnap()
    {
	return draggableSnap;
    }

    public void setDraggableSnap(String draggableSnap)
    {
	this.draggableSnap = draggableSnap;
    }

    public String getDraggableSnapMode()
    {
	return draggableSnapMode;
    }

    public void setDraggableSnapMode(String draggableSnapMode)
    {
	this.draggableSnapMode = draggableSnapMode;
    }

    public String getDraggableSnapTolerance()
    {
	return draggableSnapTolerance;
    }

    public void setDraggableSnapTolerance(String draggableSnapTolerance)
    {
	this.draggableSnapTolerance = draggableSnapTolerance;
    }

    public String getDraggableZindex()
    {
	return draggableZindex;
    }

    public void setDraggableZindex(String draggableZindex)
    {
	this.draggableZindex = draggableZindex;
    }

    public String getDroppable()
    {
	return droppable;
    }

    public void setDroppable(String droppable)
    {
	this.droppable = droppable;
    }

    public String getDroppableAccept()
    {
	return droppableAccept;
    }

    public void setDroppableAccept(String droppableAccept)
    {
	this.droppableAccept = droppableAccept;
    }

    public String getDroppableActiveClass()
    {
	return droppableActiveClass;
    }

    public void setDroppableActiveClass(String droppableActiveClass)
    {
	this.droppableActiveClass = droppableActiveClass;
    }

    public String getDroppableAddClasses()
    {
	return droppableAddClasses;
    }

    public void setDroppableAddClasses(String droppableAddClasses)
    {
	this.droppableAddClasses = droppableAddClasses;
    }

    public String getDroppableGreedy()
    {
	return droppableGreedy;
    }

    public void setDroppableGreedy(String droppableGreedy)
    {
	this.droppableGreedy = droppableGreedy;
    }

    public String getDroppableHoverClass()
    {
	return droppableHoverClass;
    }

    public void setDroppableHoverClass(String droppableHoverClass)
    {
	this.droppableHoverClass = droppableHoverClass;
    }

    public String getDroppableOnActivateTopics()
    {
	return droppableOnActivateTopics;
    }

    public void setDroppableOnActivateTopics(String droppableOnActivateTopics)
    {
	this.droppableOnActivateTopics = droppableOnActivateTopics;
    }

    public String getDroppableOnDeactivateTopics()
    {
	return droppableOnDeactivateTopics;
    }

    public void setDroppableOnDeactivateTopics(String droppableOnDeactivateTopics)
    {
	this.droppableOnDeactivateTopics = droppableOnDeactivateTopics;
    }

    public String getDroppableOnDropTopics()
    {
	return droppableOnDropTopics;
    }

    public void setDroppableOnDropTopics(String droppableOnDropTopics)
    {
	this.droppableOnDropTopics = droppableOnDropTopics;
    }

    public String getDroppableOnOutTopics()
    {
	return droppableOnOutTopics;
    }

    public void setDroppableOnOutTopics(String droppableOnOutTopics)
    {
	this.droppableOnOutTopics = droppableOnOutTopics;
    }

    public String getDroppableOnOverTopics()
    {
	return droppableOnOverTopics;
    }

    public void setDroppableOnOverTopics(String droppableOnOverTopics)
    {
	this.droppableOnOverTopics = droppableOnOverTopics;
    }

    public String getDroppableScope()
    {
	return droppableScope;
    }

    public void setDroppableScope(String droppableScope)
    {
	this.droppableScope = droppableScope;
    }

    public String getDroppableTolerance()
    {
	return droppableTolerance;
    }

    public void setDroppableTolerance(String droppableTolerance)
    {
	this.droppableTolerance = droppableTolerance;
    }

    public String getEditinline()
    {
	return editinline;
    }

    public void setEditinline(String editinline)
    {
	this.editinline = editinline;
    }

    public String getEditurl()
    {
	return editurl;
    }

    public void setEditurl(String editurl)
    {
	this.editurl = editurl;
    }

    public String getEffect()
    {
	return effect;
    }

    public void setEffect(String effect)
    {
	this.effect = effect;
    }

    public String getEffectDuration()
    {
	return effectDuration;
    }

    public void setEffectDuration(String effectDuration)
    {
	this.effectDuration = effectDuration;
    }

    public String getEffectMode()
    {
	return effectMode;
    }

    public void setEffectMode(String effectMode)
    {
	this.effectMode = effectMode;
    }

    public String getEffectOptions()
    {
	return effectOptions;
    }

    public void setEffectOptions(String effectOptions)
    {
	this.effectOptions = effectOptions;
    }

    public String getErrorElementId()
    {
	return errorElementId;
    }

    public void setErrorElementId(String errorElementId)
    {
	this.errorElementId = errorElementId;
    }

    public String getErrorText()
    {
	return errorText;
    }

    public void setErrorText(String errorText)
    {
	this.errorText = errorText;
    }

    public boolean getFilter()
    {
	return filter;
    }

    public void setFilter(boolean filter)
    {
	this.filter = filter;
    }

    public String getFilterOptions()
    {
	return filterOptions;
    }

    public void setFilterOptions(String filterOptions)
    {
	this.filterOptions = filterOptions;
    }

    public String getFooterrow()
    {
	return footerrow;
    }

    public void setFooterrow(String footerrow)
    {
	this.footerrow = footerrow;
    }

    public String getFormIds()
    {
	return formIds;
    }

    public void setFormIds(String formIds)
    {
	this.formIds = formIds;
    }

    public String getGridModel()
    {
	return gridModel;
    }

    public void setGridModel(String gridModel)
    {
	this.gridModel = gridModel;
    }

    public String getGroupCollapse()
    {
	return groupCollapse;
    }

    public void setGroupCollapse(String groupCollapse)
    {
	this.groupCollapse = groupCollapse;
    }

    public String getGroupColumnShow()
    {
	return groupColumnShow;
    }

    public void setGroupColumnShow(String groupColumnShow)
    {
	this.groupColumnShow = groupColumnShow;
    }

    public String getGroupDataSorted()
    {
	return groupDataSorted;
    }

    public void setGroupDataSorted(String groupDataSorted)
    {
	this.groupDataSorted = groupDataSorted;
    }

    public String getGroupField()
    {
	return groupField;
    }

    public void setGroupField(String groupField)
    {
	this.groupField = groupField;
    }

    public String getGroupMinusIcon()
    {
	return groupMinusIcon;
    }

    public void setGroupMinusIcon(String groupMinusIcon)
    {
	this.groupMinusIcon = groupMinusIcon;
    }

    public String getGroupOrder()
    {
	return groupOrder;
    }

    public void setGroupOrder(String groupOrder)
    {
	this.groupOrder = groupOrder;
    }

    public String getGroupPlusIcon()
    {
	return groupPlusIcon;
    }

    public void setGroupPlusIcon(String groupPlusIcon)
    {
	this.groupPlusIcon = groupPlusIcon;
    }

    public String getGroupShowSummaryOnHide()
    {
	return groupShowSummaryOnHide;
    }

    public void setGroupShowSummaryOnHide(String groupShowSummaryOnHide)
    {
	this.groupShowSummaryOnHide = groupShowSummaryOnHide;
    }

    public String getGroupSummary()
    {
	return groupSummary;
    }

    public void setGroupSummary(String groupSummary)
    {
	this.groupSummary = groupSummary;
    }

    public String getGroupText()
    {
	return groupText;
    }

    public void setGroupText(String groupText)
    {
	this.groupText = groupText;
    }

    public String getHeight()
    {
	return height;
    }

    public void setHeight(String height)
    {
	this.height = height;
    }

    public String getHiddengrid()
    {
	return hiddengrid;
    }

    public void setHiddengrid(String hiddengrid)
    {
	this.hiddengrid = hiddengrid;
    }

    public String getHidegrid()
    {
	return hidegrid;
    }

    public void setHidegrid(String hidegrid)
    {
	this.hidegrid = hidegrid;
    }

    public String getHoverrows()
    {
	return hoverrows;
    }

    public void setHoverrows(String hoverrows)
    {
	this.hoverrows = hoverrows;
    }

    public String getHref()
    {
	return href;
    }

    public void setHref(String href)
    {
	this.href = href;
    }

    public String getId()
    {
	return id;
    }

    public void setId(String id)
    {
	this.id = id;
    }

    public String getIndicator()
    {
	return indicator;
    }

    public void setIndicator(String indicator)
    {
	this.indicator = indicator;
    }

    public String getJavascriptTooltip()
    {
	return javascriptTooltip;
    }

    public void setJavascriptTooltip(String javascriptTooltip)
    {
	this.javascriptTooltip = javascriptTooltip;
    }

    public String getKey()
    {
	return key;
    }

    public void setKey(String key)
    {
	this.key = key;
    }

    public String getLabel()
    {
	return label;
    }

    public void setLabel(String label)
    {
	this.label = label;
    }

    public String getLabelSeparator()
    {
	return labelSeparator;
    }

    public void setLabelSeparator(String labelSeparator)
    {
	this.labelSeparator = labelSeparator;
    }

    public String getLabelposition()
    {
	return labelposition;
    }

    public void setLabelposition(String labelposition)
    {
	this.labelposition = labelposition;
    }

    public String getListenTopics()
    {
	return listenTopics;
    }

    public void setListenTopics(String listenTopics)
    {
	this.listenTopics = listenTopics;
    }

    public String getLoadingText()
    {
	return loadingText;
    }

    public void setLoadingText(String loadingText)
    {
	this.loadingText = loadingText;
    }

    public String getLoadonce()
    {
	return loadonce;
    }

    public void setLoadonce(String loadonce)
    {
	this.loadonce = loadonce;
    }

    public String getMultiboxonly()
    {
	return multiboxonly;
    }

    public void setMultiboxonly(String multiboxonly)
    {
	this.multiboxonly = multiboxonly;
    }

    public String getMultiselect()
    {
	return multiselect;
    }

    public void setMultiselect(String multiselect)
    {
	this.multiselect = multiselect;
    }

    public String getMultiselectWidth()
    {
	return multiselectWidth;
    }

    public void setMultiselectWidth(String multiselectWidth)
    {
	this.multiselectWidth = multiselectWidth;
    }

    public String getName()
    {
	return name;
    }

    public void setName(String name)
    {
	this.name = name;
    }

    public String getNavigator()
    {
	return navigator;
    }

    public void setNavigator(String navigator)
    {
	this.navigator = navigator;
    }

    public String getNavigatorAdd()
    {
	return navigatorAdd;
    }

    public void setNavigatorAdd(String navigatorAdd)
    {
	this.navigatorAdd = navigatorAdd;
    }

    public String getNavigatorAddOptions()
    {
	return navigatorAddOptions;
    }

    public void setNavigatorAddOptions(String navigatorAddOptions)
    {
	this.navigatorAddOptions = navigatorAddOptions;
    }

    public String getNavigatorDelete()
    {
	return navigatorDelete;
    }

    public void setNavigatorDelete(String navigatorDelete)
    {
	this.navigatorDelete = navigatorDelete;
    }

    public String getNavigatorDeleteOptions()
    {
	return navigatorDeleteOptions;
    }

    public void setNavigatorDeleteOptions(String navigatorDeleteOptions)
    {
	this.navigatorDeleteOptions = navigatorDeleteOptions;
    }

    public String getNavigatorEdit()
    {
	return navigatorEdit;
    }

    public void setNavigatorEdit(String navigatorEdit)
    {
	this.navigatorEdit = navigatorEdit;
    }

    public String getNavigatorEditOptions()
    {
	return navigatorEditOptions;
    }

    public void setNavigatorEditOptions(String navigatorEditOptions)
    {
	this.navigatorEditOptions = navigatorEditOptions;
    }

    public String getNavigatorRefresh()
    {
	return navigatorRefresh;
    }

    public void setNavigatorRefresh(String navigatorRefresh)
    {
	this.navigatorRefresh = navigatorRefresh;
    }

    public String getNavigatorSearch()
    {
	return navigatorSearch;
    }

    public void setNavigatorSearch(String navigatorSearch)
    {
	this.navigatorSearch = navigatorSearch;
    }

    public String getNavigatorSearchOptions()
    {
	return navigatorSearchOptions;
    }

    public void setNavigatorSearchOptions(String navigatorSearchOptions)
    {
	this.navigatorSearchOptions = navigatorSearchOptions;
    }

    public String getNavigatorView()
    {
	return navigatorView;
    }

    public void setNavigatorView(String navigatorView)
    {
	this.navigatorView = navigatorView;
    }

    public String getNavigatorViewOptions()
    {
	return navigatorViewOptions;
    }

    public void setNavigatorViewOptions(String navigatorViewOptions)
    {
	this.navigatorViewOptions = navigatorViewOptions;
    }

    public String getOnAlwaysTopics()
    {
	return onAlwaysTopics;
    }

    public void setOnAlwaysTopics(String onAlwaysTopics)
    {
	this.onAlwaysTopics = onAlwaysTopics;
    }

    public String getOnBeforeTopics()
    {
	return onBeforeTopics;
    }

    public void setOnBeforeTopics(String onBeforeTopics)
    {
	this.onBeforeTopics = onBeforeTopics;
    }

    public String getOnBlurTopics()
    {
	return onBlurTopics;
    }

    public void setOnBlurTopics(String onBlurTopics)
    {
	this.onBlurTopics = onBlurTopics;
    }

    public String getOnCellEditErrorTopics()
    {
	return onCellEditErrorTopics;
    }

    public void setOnCellEditErrorTopics(String onCellEditErrorTopics)
    {
	this.onCellEditErrorTopics = onCellEditErrorTopics;
    }

    public String getOnCellEditSuccessTopics()
    {
	return onCellEditSuccessTopics;
    }

    public void setOnCellEditSuccessTopics(String onCellEditSuccessTopics)
    {
	this.onCellEditSuccessTopics = onCellEditSuccessTopics;
    }

    public String getOnCellSelectTopics()
    {
	return onCellSelectTopics;
    }

    public void setOnCellSelectTopics(String onCellSelectTopics)
    {
	this.onCellSelectTopics = onCellSelectTopics;
    }

    public String getOnChangeTopics()
    {
	return onChangeTopics;
    }

    public void setOnChangeTopics(String onChangeTopics)
    {
	this.onChangeTopics = onChangeTopics;
    }

    public String getOnCompleteTopics()
    {
	return onCompleteTopics;
    }

    public void setOnCompleteTopics(String onCompleteTopics)
    {
	this.onCompleteTopics = onCompleteTopics;
    }

    public String getOnDisableTopics()
    {
	return onDisableTopics;
    }

    public void setOnDisableTopics(String onDisableTopics)
    {
	this.onDisableTopics = onDisableTopics;
    }

    public String getOnEditInlineAfterSaveTopics()
    {
	return onEditInlineAfterSaveTopics;
    }

    public void setOnEditInlineAfterSaveTopics(String onEditInlineAfterSaveTopics)
    {
	this.onEditInlineAfterSaveTopics = onEditInlineAfterSaveTopics;
    }

    public String getOnEditInlineBeforeTopics()
    {
	return onEditInlineBeforeTopics;
    }

    public void setOnEditInlineBeforeTopics(String onEditInlineBeforeTopics)
    {
	this.onEditInlineBeforeTopics = onEditInlineBeforeTopics;
    }

    public String getOnEditInlineErrorTopics()
    {
	return onEditInlineErrorTopics;
    }

    public void setOnEditInlineErrorTopics(String onEditInlineErrorTopics)
    {
	this.onEditInlineErrorTopics = onEditInlineErrorTopics;
    }

    public String getOnEditInlineSuccessTopics()
    {
	return onEditInlineSuccessTopics;
    }

    public void setOnEditInlineSuccessTopics(String onEditInlineSuccessTopics)
    {
	this.onEditInlineSuccessTopics = onEditInlineSuccessTopics;
    }

    public String getOnEffectCompleteTopics()
    {
	return onEffectCompleteTopics;
    }

    public void setOnEffectCompleteTopics(String onEffectCompleteTopics)
    {
	this.onEffectCompleteTopics = onEffectCompleteTopics;
    }

    public String getOnEnableTopics()
    {
	return onEnableTopics;
    }

    public void setOnEnableTopics(String onEnableTopics)
    {
	this.onEnableTopics = onEnableTopics;
    }

    public String getOnErrorTopics()
    {
	return onErrorTopics;
    }

    public void setOnErrorTopics(String onErrorTopics)
    {
	this.onErrorTopics = onErrorTopics;
    }

    public String getOnFocusTopics()
    {
	return onFocusTopics;
    }

    public void setOnFocusTopics(String onFocusTopics)
    {
	this.onFocusTopics = onFocusTopics;
    }

    public String getOnGridCompleteTopics()
    {
	return onGridCompleteTopics;
    }

    public void setOnGridCompleteTopics(String onGridCompleteTopics)
    {
	this.onGridCompleteTopics = onGridCompleteTopics;
    }

    public String getOnPagingTopics()
    {
	return onPagingTopics;
    }

    public void setOnPagingTopics(String onPagingTopics)
    {
	this.onPagingTopics = onPagingTopics;
    }

    public String getOnSelectAllTopics()
    {
	return onSelectAllTopics;
    }

    public void setOnSelectAllTopics(String onSelectAllTopics)
    {
	this.onSelectAllTopics = onSelectAllTopics;
    }

    public String getOnSelectRowTopics()
    {
	return onSelectRowTopics;
    }

    public void setOnSelectRowTopics(String onSelectRowTopics)
    {
	this.onSelectRowTopics = onSelectRowTopics;
    }

    public String getOnSortColTopics()
    {
	return onSortColTopics;
    }

    public void setOnSortColTopics(String onSortColTopics)
    {
	this.onSortColTopics = onSortColTopics;
    }

    public String getOnSuccessTopics()
    {
	return onSuccessTopics;
    }

    public void setOnSuccessTopics(String onSuccessTopics)
    {
	this.onSuccessTopics = onSuccessTopics;
    }

    public String getOnblur()
    {
	return onblur;
    }

    public void setOnblur(String onblur)
    {
	this.onblur = onblur;
    }

    public String getOnchange()
    {
	return onchange;
    }

    public void setOnchange(String onchange)
    {
	this.onchange = onchange;
    }

    public String getOnclick()
    {
	return onclick;
    }

    public void setOnclick(String onclick)
    {
	this.onclick = onclick;
    }

    public String getOndblclick()
    {
	return ondblclick;
    }

    public void setOndblclick(String ondblclick)
    {
	this.ondblclick = ondblclick;
    }

    public String getOnfocus()
    {
	return onfocus;
    }

    public void setOnfocus(String onfocus)
    {
	this.onfocus = onfocus;
    }

    public String getOnkeydown()
    {
	return onkeydown;
    }

    public void setOnkeydown(String onkeydown)
    {
	this.onkeydown = onkeydown;
    }

    public String getOnkeypress()
    {
	return onkeypress;
    }

    public void setOnkeypress(String onkeypress)
    {
	this.onkeypress = onkeypress;
    }

    public String getOnkeyup()
    {
	return onkeyup;
    }

    public void setOnkeyup(String onkeyup)
    {
	this.onkeyup = onkeyup;
    }

    public String getOnmousedown()
    {
	return onmousedown;
    }

    public void setOnmousedown(String onmousedown)
    {
	this.onmousedown = onmousedown;
    }

    public String getOnmousemove()
    {
	return onmousemove;
    }

    public void setOnmousemove(String onmousemove)
    {
	this.onmousemove = onmousemove;
    }

    public String getOnmouseout()
    {
	return onmouseout;
    }

    public void setOnmouseout(String onmouseout)
    {
	this.onmouseout = onmouseout;
    }

    public String getOnmouseover()
    {
	return onmouseover;
    }

    public void setOnmouseover(String onmouseover)
    {
	this.onmouseover = onmouseover;
    }

    public String getOnmouseup()
    {
	return onmouseup;
    }

    public void setOnmouseup(String onmouseup)
    {
	this.onmouseup = onmouseup;
    }

    public String getOnselect()
    {
	return onselect;
    }

    public void setOnselect(String onselect)
    {
	this.onselect = onselect;
    }

    public String getOpenTemplate()
    {
	return openTemplate;
    }

    public void setOpenTemplate(String openTemplate)
    {
	this.openTemplate = openTemplate;
    }

    public String getPage()
    {
	return page;
    }

    public void setPage(String page)
    {
	this.page = page;
    }

    public String getPager()
    {
	return pager;
    }

    public void setPager(String pager)
    {
	this.pager = pager;
    }

    public String getPagerButtons()
    {
	return pagerButtons;
    }

    public void setPagerButtons(String pagerButtons)
    {
	this.pagerButtons = pagerButtons;
    }

    public String getPagerInput()
    {
	return pagerInput;
    }

    public void setPagerInput(String pagerInput)
    {
	this.pagerInput = pagerInput;
    }

    public String getPagerPosition()
    {
	return pagerPosition;
    }

    public void setPagerPosition(String pagerPosition)
    {
	this.pagerPosition = pagerPosition;
    }

    public String getPrmNames()
    {
	return prmNames;
    }

    public void setPrmNames(String prmNames)
    {
	this.prmNames = prmNames;
    }

    public String getRecordpos()
    {
	return recordpos;
    }

    public void setRecordpos(String recordpos)
    {
	this.recordpos = recordpos;
    }

    public String getReloadTopics()
    {
	return reloadTopics;
    }

    public void setReloadTopics(String reloadTopics)
    {
	this.reloadTopics = reloadTopics;
    }

    public String getRequestType()
    {
	return requestType;
    }

    public void setRequestType(String requestType)
    {
	this.requestType = requestType;
    }

    public String getRequired()
    {
	return required;
    }

    public void setRequired(String required)
    {
	this.required = required;
    }

    public String getRequiredposition()
    {
	return requiredposition;
    }

    public void setRequiredposition(String requiredposition)
    {
	this.requiredposition = requiredposition;
    }

    public String getResizable()
    {
	return resizable;
    }

    public void setResizable(String resizable)
    {
	this.resizable = resizable;
    }

    public String getResizableAnimate()
    {
	return resizableAnimate;
    }

    public void setResizableAnimate(String resizableAnimate)
    {
	this.resizableAnimate = resizableAnimate;
    }

    public String getResizableAnimateDuration()
    {
	return resizableAnimateDuration;
    }

    public void setResizableAnimateDuration(String resizableAnimateDuration)
    {
	this.resizableAnimateDuration = resizableAnimateDuration;
    }

    public String getResizableAnimateEasing()
    {
	return resizableAnimateEasing;
    }

    public void setResizableAnimateEasing(String resizableAnimateEasing)
    {
	this.resizableAnimateEasing = resizableAnimateEasing;
    }

    public String getResizableAspectRatio()
    {
	return resizableAspectRatio;
    }

    public void setResizableAspectRatio(String resizableAspectRatio)
    {
	this.resizableAspectRatio = resizableAspectRatio;
    }

    public String getResizableAutoHide()
    {
	return resizableAutoHide;
    }

    public void setResizableAutoHide(String resizableAutoHide)
    {
	this.resizableAutoHide = resizableAutoHide;
    }

    public String getResizableContainment()
    {
	return resizableContainment;
    }

    public void setResizableContainment(String resizableContainment)
    {
	this.resizableContainment = resizableContainment;
    }

    public String getResizableDelay()
    {
	return resizableDelay;
    }

    public void setResizableDelay(String resizableDelay)
    {
	this.resizableDelay = resizableDelay;
    }

    public String getResizableDistance()
    {
	return resizableDistance;
    }

    public void setResizableDistance(String resizableDistance)
    {
	this.resizableDistance = resizableDistance;
    }

    public String getResizableGhost()
    {
	return resizableGhost;
    }

    public void setResizableGhost(String resizableGhost)
    {
	this.resizableGhost = resizableGhost;
    }

    public String getResizableHandles()
    {
	return resizableHandles;
    }

    public void setResizableHandles(String resizableHandles)
    {
	this.resizableHandles = resizableHandles;
    }

    public String getResizableHelper()
    {
	return resizableHelper;
    }

    public void setResizableHelper(String resizableHelper)
    {
	this.resizableHelper = resizableHelper;
    }

    public String getResizableMaxHeight()
    {
	return resizableMaxHeight;
    }

    public void setResizableMaxHeight(String resizableMaxHeight)
    {
	this.resizableMaxHeight = resizableMaxHeight;
    }

    public String getResizableMaxWidth()
    {
	return resizableMaxWidth;
    }

    public void setResizableMaxWidth(String resizableMaxWidth)
    {
	this.resizableMaxWidth = resizableMaxWidth;
    }

    public String getResizableMinHeight()
    {
	return resizableMinHeight;
    }

    public void setResizableMinHeight(String resizableMinHeight)
    {
	this.resizableMinHeight = resizableMinHeight;
    }

    public String getResizableMinWidth()
    {
	return resizableMinWidth;
    }

    public void setResizableMinWidth(String resizableMinWidth)
    {
	this.resizableMinWidth = resizableMinWidth;
    }

    public String getResizableOnResizeTopics()
    {
	return resizableOnResizeTopics;
    }

    public void setResizableOnResizeTopics(String resizableOnResizeTopics)
    {
	this.resizableOnResizeTopics = resizableOnResizeTopics;
    }

    public String getResizableOnStartTopics()
    {
	return resizableOnStartTopics;
    }

    public void setResizableOnStartTopics(String resizableOnStartTopics)
    {
	this.resizableOnStartTopics = resizableOnStartTopics;
    }

    public String getResizableOnStopTopics()
    {
	return resizableOnStopTopics;
    }

    public void setResizableOnStopTopics(String resizableOnStopTopics)
    {
	this.resizableOnStopTopics = resizableOnStopTopics;
    }

    public String[] getRowList()
    {
	return rowList;
    }

    public void setRowList(String... rowList)
    {
	this.rowList = rowList;
    }

    public String getRowNum()
    {
	return rowNum;
    }

    public void setRowNum(String rowNum)
    {
	this.rowNum = rowNum;
    }

    public String getRowTotal()
    {
	return rowTotal;
    }

    public void setRowTotal(String rowTotal)
    {
	this.rowTotal = rowTotal;
    }

    public String getRownumbers()
    {
	return rownumbers;
    }

    public void setRownumbers(String rownumbers)
    {
	this.rownumbers = rownumbers;
    }

    public String getScroll()
    {
	return scroll;
    }

    public void setScroll(String scroll)
    {
	this.scroll = scroll;
    }

    public String getScrollrows()
    {
	return scrollrows;
    }

    public void setScrollrows(String scrollrows)
    {
	this.scrollrows = scrollrows;
    }

    public String getShrinkToFit()
    {
	return shrinkToFit;
    }

    public void setShrinkToFit(String shrinkToFit)
    {
	this.shrinkToFit = shrinkToFit;
    }

    public String getSortable()
    {
	return sortable;
    }

    public void setSortable(String sortable)
    {
	this.sortable = sortable;
    }

    public String getSortableAppendTo()
    {
	return sortableAppendTo;
    }

    public void setSortableAppendTo(String sortableAppendTo)
    {
	this.sortableAppendTo = sortableAppendTo;
    }

    public String getSortableAxis()
    {
	return sortableAxis;
    }

    public void setSortableAxis(String sortableAxis)
    {
	this.sortableAxis = sortableAxis;
    }

    public String getSortableCancel()
    {
	return sortableCancel;
    }

    public void setSortableCancel(String sortableCancel)
    {
	this.sortableCancel = sortableCancel;
    }

    public String getSortableConnectWith()
    {
	return sortableConnectWith;
    }

    public void setSortableConnectWith(String sortableConnectWith)
    {
	this.sortableConnectWith = sortableConnectWith;
    }

    public String getSortableContainment()
    {
	return sortableContainment;
    }

    public void setSortableContainment(String sortableContainment)
    {
	this.sortableContainment = sortableContainment;
    }

    public String getSortableCursor()
    {
	return sortableCursor;
    }

    public void setSortableCursor(String sortableCursor)
    {
	this.sortableCursor = sortableCursor;
    }

    public String getSortableCursorAt()
    {
	return sortableCursorAt;
    }

    public void setSortableCursorAt(String sortableCursorAt)
    {
	this.sortableCursorAt = sortableCursorAt;
    }

    public String getSortableDelay()
    {
	return sortableDelay;
    }

    public void setSortableDelay(String sortableDelay)
    {
	this.sortableDelay = sortableDelay;
    }

    public String getSortableDistance()
    {
	return sortableDistance;
    }

    public void setSortableDistance(String sortableDistance)
    {
	this.sortableDistance = sortableDistance;
    }

    public String getSortableDropOnEmpty()
    {
	return sortableDropOnEmpty;
    }

    public void setSortableDropOnEmpty(String sortableDropOnEmpty)
    {
	this.sortableDropOnEmpty = sortableDropOnEmpty;
    }

    public String getSortableForceHelperSize()
    {
	return sortableForceHelperSize;
    }

    public void setSortableForceHelperSize(String sortableForceHelperSize)
    {
	this.sortableForceHelperSize = sortableForceHelperSize;
    }

    public String getSortableForcePlaceholderSize()
    {
	return sortableForcePlaceholderSize;
    }

    public void setSortableForcePlaceholderSize(String sortableForcePlaceholderSize)
    {
	this.sortableForcePlaceholderSize = sortableForcePlaceholderSize;
    }

    public String getSortableGrid()
    {
	return sortableGrid;
    }

    public void setSortableGrid(String sortableGrid)
    {
	this.sortableGrid = sortableGrid;
    }

    public String getSortableHandle()
    {
	return sortableHandle;
    }

    public void setSortableHandle(String sortableHandle)
    {
	this.sortableHandle = sortableHandle;
    }

    public String getSortableHelper()
    {
	return sortableHelper;
    }

    public void setSortableHelper(String sortableHelper)
    {
	this.sortableHelper = sortableHelper;
    }

    public String getSortableItems()
    {
	return sortableItems;
    }

    public void setSortableItems(String sortableItems)
    {
	this.sortableItems = sortableItems;
    }

    public String getSortableOnActivateTopics()
    {
	return sortableOnActivateTopics;
    }

    public void setSortableOnActivateTopics(String sortableOnActivateTopics)
    {
	this.sortableOnActivateTopics = sortableOnActivateTopics;
    }

    public String getSortableOnBeforeStopTopics()
    {
	return sortableOnBeforeStopTopics;
    }

    public void setSortableOnBeforeStopTopics(String sortableOnBeforeStopTopics)
    {
	this.sortableOnBeforeStopTopics = sortableOnBeforeStopTopics;
    }

    public String getSortableOnChangeTopics()
    {
	return sortableOnChangeTopics;
    }

    public void setSortableOnChangeTopics(String sortableOnChangeTopics)
    {
	this.sortableOnChangeTopics = sortableOnChangeTopics;
    }

    public String getSortableOnDeactivateTopics()
    {
	return sortableOnDeactivateTopics;
    }

    public void setSortableOnDeactivateTopics(String sortableOnDeactivateTopics)
    {
	this.sortableOnDeactivateTopics = sortableOnDeactivateTopics;
    }

    public String getSortableOnOutTopics()
    {
	return sortableOnOutTopics;
    }

    public void setSortableOnOutTopics(String sortableOnOutTopics)
    {
	this.sortableOnOutTopics = sortableOnOutTopics;
    }

    public String getSortableOnOverTopics()
    {
	return sortableOnOverTopics;
    }

    public void setSortableOnOverTopics(String sortableOnOverTopics)
    {
	this.sortableOnOverTopics = sortableOnOverTopics;
    }

    public String getSortableOnReceiveTopics()
    {
	return sortableOnReceiveTopics;
    }

    public void setSortableOnReceiveTopics(String sortableOnReceiveTopics)
    {
	this.sortableOnReceiveTopics = sortableOnReceiveTopics;
    }

    public String getSortableOnRemoveTopics()
    {
	return sortableOnRemoveTopics;
    }

    public void setSortableOnRemoveTopics(String sortableOnRemoveTopics)
    {
	this.sortableOnRemoveTopics = sortableOnRemoveTopics;
    }

    public String getSortableOnSortTopics()
    {
	return sortableOnSortTopics;
    }

    public void setSortableOnSortTopics(String sortableOnSortTopics)
    {
	this.sortableOnSortTopics = sortableOnSortTopics;
    }

    public String getSortableOnStartTopics()
    {
	return sortableOnStartTopics;
    }

    public void setSortableOnStartTopics(String sortableOnStartTopics)
    {
	this.sortableOnStartTopics = sortableOnStartTopics;
    }

    public String getSortableOnStopTopics()
    {
	return sortableOnStopTopics;
    }

    public void setSortableOnStopTopics(String sortableOnStopTopics)
    {
	this.sortableOnStopTopics = sortableOnStopTopics;
    }

    public String getSortableOnUpdateTopics()
    {
	return sortableOnUpdateTopics;
    }

    public void setSortableOnUpdateTopics(String sortableOnUpdateTopics)
    {
	this.sortableOnUpdateTopics = sortableOnUpdateTopics;
    }

    public String getSortableOpacity()
    {
	return sortableOpacity;
    }

    public void setSortableOpacity(String sortableOpacity)
    {
	this.sortableOpacity = sortableOpacity;
    }

    public String getSortablePlaceholder()
    {
	return sortablePlaceholder;
    }

    public void setSortablePlaceholder(String sortablePlaceholder)
    {
	this.sortablePlaceholder = sortablePlaceholder;
    }

    public String getSortableRevert()
    {
	return sortableRevert;
    }

    public void setSortableRevert(String sortableRevert)
    {
	this.sortableRevert = sortableRevert;
    }

    public String getSortableScroll()
    {
	return sortableScroll;
    }

    public void setSortableScroll(String sortableScroll)
    {
	this.sortableScroll = sortableScroll;
    }

    public String getSortableScrollSensitivity()
    {
	return sortableScrollSensitivity;
    }

    public void setSortableScrollSensitivity(String sortableScrollSensitivity)
    {
	this.sortableScrollSensitivity = sortableScrollSensitivity;
    }

    public String getSortableScrollSpeed()
    {
	return sortableScrollSpeed;
    }

    public void setSortableScrollSpeed(String sortableScrollSpeed)
    {
	this.sortableScrollSpeed = sortableScrollSpeed;
    }

    public String getSortableTolerance()
    {
	return sortableTolerance;
    }

    public void setSortableTolerance(String sortableTolerance)
    {
	this.sortableTolerance = sortableTolerance;
    }

    public String getSortableZindex()
    {
	return sortableZindex;
    }

    public void setSortableZindex(String sortableZindex)
    {
	this.sortableZindex = sortableZindex;
    }

    public String getSortname()
    {
	return sortname;
    }

    public void setSortname(String sortname)
    {
	this.sortname = sortname;
    }

    public String getSortorder()
    {
	return sortorder;
    }

    public void setSortorder(String sortorder)
    {
	this.sortorder = sortorder;
    }

    public String getSubGridUrl()
    {
	return subGridUrl;
    }

    public void setSubGridUrl(String subGridUrl)
    {
	this.subGridUrl = subGridUrl;
    }

    public String getSubGridWidth()
    {
	return subGridWidth;
    }

    public void setSubGridWidth(String subGridWidth)
    {
	this.subGridWidth = subGridWidth;
    }

    public String getTabindex()
    {
	return tabindex;
    }

    public void setTabindex(String tabindex)
    {
	this.tabindex = tabindex;
    }

    public String getTargets()
    {
	return targets;
    }

    public void setTargets(String targets)
    {
	this.targets = targets;
    }

    public String getTemplate()
    {
	return template;
    }

    public void setTemplate(String template)
    {
	this.template = template;
    }

    public String getTemplateDir()
    {
	return templateDir;
    }

    public void setTemplateDir(String templateDir)
    {
	this.templateDir = templateDir;
    }

    public String getTimeout()
    {
	return timeout;
    }

    public void setTimeout(String timeout)
    {
	this.timeout = timeout;
    }

    public String getTitle()
    {
	return title;
    }

    public void setTitle(String title)
    {
	this.title = title;
    }

    public String getTooltip()
    {
	return tooltip;
    }

    public void setTooltip(String tooltip)
    {
	this.tooltip = tooltip;
    }

    public String getTooltipConfig()
    {
	return tooltipConfig;
    }

    public void setTooltipConfig(String tooltipConfig)
    {
	this.tooltipConfig = tooltipConfig;
    }

    public String getTooltipCssClass()
    {
	return tooltipCssClass;
    }

    public void setTooltipCssClass(String tooltipCssClass)
    {
	this.tooltipCssClass = tooltipCssClass;
    }

    public String getTooltipDelay()
    {
	return tooltipDelay;
    }

    public void setTooltipDelay(String tooltipDelay)
    {
	this.tooltipDelay = tooltipDelay;
    }

    public String getTooltipIconPath()
    {
	return tooltipIconPath;
    }

    public void setTooltipIconPath(String tooltipIconPath)
    {
	this.tooltipIconPath = tooltipIconPath;
    }

    public String getUserDataOnFooter()
    {
	return userDataOnFooter;
    }

    public void setUserDataOnFooter(String userDataOnFooter)
    {
	this.userDataOnFooter = userDataOnFooter;
    }

    public String getValue()
    {
	return value;
    }

    public void setValue(String value)
    {
	this.value = value;
    }

    public Boolean getViewrecords()
    {
	return viewrecords;
    }

    public void setViewrecords(Boolean viewrecords)
    {
	this.viewrecords = viewrecords;
    }

    public String getWidth()
    {
	return width;
    }

    public void setWidth(String width)
    {
	this.width = width;
    }

    public String getUrl()
    {
	return url;
    }

    public void setUrl(String url)
    {
	this.url = url;
    }

    public String getSearchColumns()
    {
        return searchColumns;
    }

    public void setSearchColumns(String searchColumns)
    {
        this.searchColumns = searchColumns;
    }

    /**
     * @return the columnsOrder
     */
    public String getColumnsOrder()
    {
        return columnsOrder;
    }

    /**
     * @param columnsOrder the columnsOrder to set
     */
    public void setColumnsOrder(String columnsOrder)
    {
        this.columnsOrder = columnsOrder;
    }
}
